...
export default class UserController implements Controller {
  ...
  private getByUserName = async (req: Request, res: Response) => {
    try {
      ...
      if (data.length > 0) {
        const body = { log: `${username} user loged in!`, date: new Date().toLocaleString("hu-HU", {timeZone: "UTC+2"}) };
        ...
        const body = { log: `${username} user tried to log in!`, date: new Date().toLocaleString("hu-HU", {timeZone: "UTC+2"}) };
        const createdDocument = new this.log({
          ...body
        });
        createdDocument["_id"] = new mongoose.Types.ObjectId();
